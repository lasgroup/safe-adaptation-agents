...
def solve(objective_fn: Callable[[jnp.ndarray], jnp.ndarray],
  ...
  def body(val):
    ...
    best = jnp.where(scores[elite_ids[-1]] > best_score, sample[elite_ids[-1]],
                     best)
  ...
  *_, best = jax.lax.while_loop(cond, body,
                                (key, 0, mu, stddev, -jnp.inf, initial_guess))
  return best
